...
| **四 · leaderboard** | §15–§20 | 预算换算 → 定 lr → 定形状 → 跑满；再换固定-token 口径跑 0.6B | $\text{lr}^\ast\propto1/d$；宽深比碗底在 **64**；最终 **val 3.2786**；**换口径赢家就反过来** |
| **方法** | §21–§22 | 实现与验证的流程、全局教训 | 验证的五级台阶；每个 bug"该被谁抓 vs 实际被谁抓" |
...
| | **── 换口径：固定 token = OWT 一整遍（§20）──** |||||||||
| 55 | 一整遍 | $d$1408 $L$22 | 64 | **570.5 M** | 8.18e-4 | 9.59e18 | 41613 | **2.9845**（BPB 0.987） | `owt1ep_06B` |
...
# 20. 换个口径：固定 token 而不是固定算力

## 为什么做这个实验

前面所有 leaderboard 的推理都建立在**固定算力**上（45 分钟 B200）。但"32 GB 显存能装 0.6B，为什么只选 100M"这个问题指向另一个口径：**如果不限时间、只把 OWT 完整过一遍呢？**

这两个口径的性质完全不同：

| | 固定算力 $C\approx6ND$ | 固定 token $D$ |
| --- | --- | --- |
| 模型越大 | 能看的 token 越少 | 不受影响 |
| 有没有最优点 | **有**（$N^\ast=\sqrt{C/120}$） | **没有，越大越好** |
| 要问的问题 | 该配多大 | 值不值那个算力 |

## 步骤

**步骤 1：定架构。** 沿用前面测出来的两条规律——$d/L$=64（§18 的碗底）、$\text{lr}^\ast\approx1.5\text{e-}3\cdot768/d$（§17）。$d$=1408 / $L$=22 / $d_{ff}$=3776 / tied → **570.5 M**，lr = 8.18e-4。

**步骤 2：冒烟 2 步确认显存和吞吐。** 峰值 18.66 GB（32 GB 卡装得下），36.5k tok/s，**MFU 61.4%**——全项目最高。

**步骤 3：跑满一整遍。** 2,727,120,452 token ÷ (有效 batch 256 × 上下文 256) = **41,613 步**。

> **一个没做的保险**：lr 是从 $d\in[768,1024]$ 外推到 $d$=1408 的。本来该先跑 3 条短探针（40 分钟）验一下，跳过了。结果没出事，但这属于运气——§16 那条 `lb_A1` 就是外推 lr 翻车的例子。

## 数据

| run | 参数 | token | tok/参数 | val | 困惑度 | BPB | 从上下文榨出 | 算力 | 墙钟 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `final_d768L12` | 109.5 M | 1.77 B | 16.1 | 3.2786 | 26.54 | 1.0841 | 4.281 | 1.21e18 | 3h18m |
| **`owt1ep_06B`** | **570.5 M** | **2.73 B** | 4.8 | **2.9845** | **19.78** | **0.9869** | **4.575** | 9.59e18 | 20h06m |

**BPB 跌破 1.0**——每个字节不到 1 bit。

![固定 token vs 固定算力](notes_img/14_fixed_token_vs_flops.png)

## 结论：同一对模型，两个口径给出相反的赢家

两条 run 的有效 batch 和上下文都一样，所以 **step 轴就是 token 轴**，可以直接对齐比：

| 口径 | 109.5M | 570.5M | 谁赢 |
| --- | --- | --- | --- |
| **固定 token**（1.70B，两条都有 eval 的最大公共点） | 3.2796 | **3.0759** | 0.6B 领先 **0.20** |
| **固定算力**（1.21e18 FLOPs） | **3.2786** | ~3.62 | 100M 领先 **0.34** |

**0.6B 每 token 贵 5.1 倍。** 同样 21 小时，109.5M 能把 OWT 跑 4.4 遍。

固定算力那一栏的 −0.34 和 §15 用 Chinchilla 预测的 **+0.31** 几乎吻合——那个模型在这个尺度上是准的。

**两个陷阱**：

**① 固定 token 那一栏还在压 0.6B。** 两条的 cosine 周期不同（26,943 vs 41,613 步），step 26000 时 109.5M 已经退火到底、lr 接近 min，而 0.6B 才走到 62%、lr 还高着。**退火尾段还能再收一截**，所以 0.6B 真实的领先幅度比 0.20 更大（跑完是 2.9845）。

**② 别用墙钟当横轴。** wandb 默认可能是 `_runtime`，那样 0.6B 会显得"一直落后"——但那是在比算力效率，不是比模型。要看固定 token 就用 `_step`。

## 一个还没补的对照

`final_d768L12` 只跑了 1.77B token（算力预算卡的），不是完整一遍。所以上表的 −0.294 里**同时混了参数量和 token 量两个变量**。

用 §14 那个校准过的 Chinchilla 估计，109.5M 跑满 2.73B 约 **3.157**，那么：

```
总差距 0.294 = 参数量的贡献 ~0.17 + 多看 0.96B token 的贡献 ~0.12
```

要把这个拆分坐实，需要补一条 109.5M × 2.73B 的 run（41,613 步 ≈ 4.8 小时）。**目前是估计，不是实测。**
