...
#### 3.4.1b 预分词也能再快（train 2.23GB，32 进程）

merge loop 榨干之后，瓶颈回到预分词。两处改动：

| # | 版本 | 2.23GB 耗时 | vs 上一步 | vs 起点 |
|---|---|---|---|---|
| 0 | `finditer` + `local_vocab[m.group()] += 1` | 12.58s | — | 1× |
| 1 | + `findall` + `Counter.update`（计数走 C） | 7.55s | **1.67×** | 1.67× |
| 2 | + 分块数 = 进程数 × 4（负载均衡） | **7.17s** | 1.05× | **1.75×** |

**#1 为什么快**：`finditer` 每个 token 都要建一个 Match 对象、再走一次 Python 层的 `+= 1`（dict 查 + 加 + 写回）。换成 `Counter.update(pattern.findall(part))` 后，计数落到 CPython 的 `_count_elements` C 函数里，整条 Python 循环消失。顺手把 `PAT` 在模块级 `re.compile` 掉。
- 中间列表不占内存：`findall` 是按**每篇文档**调的（split 之后），文档平均才 ~800 字节。
- 单进程下这条是 **1.54×**；32 进程下只有 1.29×——因为 fork/pickle/合并的固定开销占比变大了（见下）。大语料上才回到 1.67×。

**#2 为什么有效**：文档长度不均 → 每块的实际工作量不均。实测 valid 上 **最慢 worker 79.6ms vs 平均 58ms**，快的干完就空转等最慢那个。块数开到进程数的 4 倍，快 worker 能多领任务。
- ⚠️ 别开太大：512 块反而变慢（0.161s vs 128 块的 0.115s），每个任务的 pickle/调度固定开销开始吃掉收益。

**valid（22.5MB）上的开销拆解**（说明为什么小语料上优化看不出来）：

| 环节 | 耗时 | 占比 |
|---|---|---|
| Pool 创建（fork 32 进程） | 12.9 ms | 12% |
| 派发 + worker 计算 + pickle 回传 | 83.8 ms | 75%（最慢 worker 内部只算了 79.6ms） |
| 主进程合并 32 个 Counter | 11.3 ms | 10% |
| **总计** | **111.3 ms** | |

⇒ 小语料上固定开销占 ~30%，正则再快也压不下去。**优化预分词要在 GB 级语料上验证。**
...
### 3.6 owt 规模的优化工作流（2026-07-25 起）

**触发**：`train_bpe("data/owt_train.txt", 32000, ["<|endoftext|>"])` 实测
```
Pre-tokenization took  40.84 s
BPE Merge tooks       544.95 s     ← 93%，merge 重新变成绝对大头
```
和 TinyStories 完全相反（那里 pretok 6.6s / merge 2.3s）。根因是 **unique pre-token 数差了 110 倍**：TinyStories train 6 万，owt_train **660 万**（网页文本，URL/乱码/拼写错误全是独立 token）。

⚠️ **每条性能结论都必须标注测量规模**，否则会像上一轮那样直接反转。

#### 3.6.1 流程（每条优化都走一遍，不跳步）

| 步骤 | 做什么 | 判据 |
|---|---|---|
| 0. 定基线 | 在**迭代语料**上测 3 次取中位数，存下 merges 序列作为正确性基准 | — |
| 1. profile | `cProfile` 找热点，别猜 | tottime 前 10 |
| 2. 改一条 | **一次只改一个变量**，否则归因不了 | — |
| 3. 对拍 | merges 序列和基线**逐条相同** | 不同 → 直接回滚，不 debug 性能 |
| 4. 测时 | 同样 3 次取中位数 | — |
| 5. 记账 | 填进 §3.6.4 的表 | — |
| 6. 取舍 | 收益 < 5% 或复杂度涨太多 → 回滚 | — |
| 7. 收尾 | 全做完后在**目标语料**确认累积效果 | — |

**两级语料**（关键，上一轮的教训）：
- **迭代语料** = `owt_valid.txt`（290MB）+ vocab 32000 —— 几十秒一轮，且**保留 owt 的特征**（unique pre-token 多、affected 集合大）。
- **目标语料** = `owt_train.txt`（11.9GB）+ vocab 32000 —— 10 分钟一轮，只在最后确认。
- ⚠️ **绝不能用 TinyStories 当迭代语料**：它 `affected` 的中位数是 1，所有和"每轮粒度"相关的优化在上面全都测不出来。

**内存预算**：机器 128GB，全量跑实测峰值 ~19GB ⇒ 有 ~100GB 可花，**本轮明确以空间换时间**。

#### 3.6.2 基线剖析（owt_train + vocab 32000，instrument 版）

把每轮拆成 MAP（只读 `ids[wi]`，可并行）和 REDUCE（改 `pair_counts`/`pair_to_words`/heap，必须串行）：

| 指标 | mean | median | p90 | max |
|---|---|---|---|---|
| `len(affected)` | 1,135 | 60 | 1,213 | **631,850** |
| MAP 每轮 | 20.5 ms | 0.3 ms | 7.5 ms | 8,611 ms |
| REDUCE 每轮 | 6.2 ms | 0.2 ms | 6.1 ms | 6,185 ms |
| 每轮扫过的 id 总数 | 9,603 | 249 | 6,929 | 7,145,919 |

| 环节 | 耗时 | 占比 |
|---|---|---|
| **MAP**（建 new_seq + 两个 `Counter(zip(...))`） | 649.6s | **72%** |
| **REDUCE**（`pc` / `p2w` / heap 更新） | 198.2s | 22% |
| `pop_best` | 4.3s | **0.5%** |
| init（建 4 个结构） | 31.8s | — |

**四条关键读数**：
1. **热点是 MAP 的 72%，不是 heap**。`pop_best` 只占 0.5% ⇒ 「heap key 大整数化」这条优化直接**降级不做**，它优化的是 0.5%。
2. **平均词长只有 8.5 个 id**（9603/1135），但每个 affected word 每轮都要重建**两个** `Counter(zip(...))` 扫全词 ⇒ 一次 merge 通常只改词里 2~3 个 pair，却付了 O(词长) 的代价。**这是 MAP 里最大的浪费。**
3. **前 4000 次 merge 吃掉 676s / 896s（75%）**。早期 merge（` t`、`he`）的 affected 是几十万词级别。优化必须瞄准这一段。
4. 中位数 `len(affected)=60`、MAP 中位 0.3ms —— 比 TinyStories 的 3.5µs 大了两个数量级，**并行在这个规模上第一次变得可算**。

> ⚠️ instrument 版是 896.7s，真实版是 545s。差的 351s 是 map-then-reduce 结构为了「先全算完再统一提交」而物化 `work` 列表的开销（一轮最多 63 万条）。所以上表的 MAP 占比是**偏高**的估计；但真正的并行实现同样要物化结果才能传输，所以这个开销并非纯人为。

#### 3.6.3 并行的正确形状：永久分片

**直接并行 `for word_idx in affected` 是错的**：每轮都要把 `ids[wi]` pickle 进 worker、结果 pickle 回来。实测一轮最多要搬 **714 万个 id**，搬运量正比于工作量，省不下来。

**永久分片**：
- worker k 永久持有 `words[k::N]` 的 `ids` / `cnts` / **自己那份局部 `pair_to_words`**。
- 每轮父进程只广播 `(id_a, id_b, new_id)`——几十字节，和 affected 大小脱钩。
- 每个 worker 在自己分片上跑完整 affected 循环，返回**局部 pair 计数 delta 字典**。
- 父进程归并 N 个 delta → 更新 `pair_counts` + heap → `pop_best` → 再广播。

好处：`p2w` 的维护整个搬进 worker，父进程只剩 `pc` 更新 + heap push + pop_best。

**可行性判据 = 每轮粒度**：

| 规模 | 每轮 word 循环（中位数） | 派发 32 个空任务 | 结论 |
|---|---|---|---|
| TinyStories valid + vocab 10000 | 3.5 µs | 进程 300µs / 线程 132µs | 开销贵 40~85 倍，**绝无可能** |
| owt_train + vocab 32000 | 0.3 ms（均值 20.5ms） | 同上 | 开销占 1.5%，**值得算** |

**理论上限**（假设 MAP 完美 32× 且数据传输免费）：896.7s → 229.0s = **3.92×**；MAP 归零的绝对上限 4.43×。

**线程一律排除**：CPython 3.12 有 GIL，循环体是 100% Python 字节码操作 dict/tuple，线程只付开销不给加速。

#### 3.6.4 待测清单（按实测热点排序）

先把串行版榨干再并行——否则并行只是把低效代码复制 32 份。

| # | 优化 | 打击哪块 | 思路 | 内存代价 |
|---|---|---|---|---|
| O1 | 邻域增量 + per-word pair 计数常驻 | **MAP 72%** | 不再重建两个整词 Counter；只算 merge 点邻域的 2~3 个 pair 的 delta。常驻的 per-word 计数用来判断 `p2w` 成员关系（否则光有 delta 判不出来） | ~2-3 GB |
| O2 | pair 用 int 编码 | MAP + REDUCE | `(a,b)` → `a << 21 \| b`，省 tuple 分配、dict 哈希更快 | 无（更省） |
| O3 | 分片并行 | 全局 | 见 §3.6.3 | ~1.3× |
| ~~O4~~ | ~~heap key 大整数化~~ | ~~pop_best~~ | **实测只占 0.5%，取消** | — |

#### 3.6.5 结果表（迭代语料 = owt_valid 290MB + vocab 32000）

跑法：`harness.py`，3 次取中位数，**每一版都和基线的 31743 条 merge 逐条对拍**。

| # | 改动 | 耗时 | vs 上一步 | vs 起点 | 对拍 | 采纳 |
|---|---|---|---|---|---|---|
| 0 | 基线（§3.4 收尾状态） | 42.75s | — | 1× | 基准 | — |
| **G** | **merge loop 期间 `gc.disable()`** | **28.21s** | **1.52×** | 1.52× | ✅ 相同 | ✅ |
| **O1** | **邻域增量 + per-word pair 计数常驻** | **19.93s** | **1.42×** | **2.15×** | ✅ 相同 | ✅ |
| O2 | heap key 后缀按 pair 缓存 | 20.02s | 1.00× | — | ✅ 相同 | ❌ 无收益 |
| O3a | rebuild 阈值放宽到 `8×live+1M` | 27.79s | 0.72× | — | ✅ 相同 | ❌ 更差 |
| O3b | 完全不 rebuild（堆无限长） | 51.82s | 0.38× | — | ✅ 相同 | ❌ 灾难 |
| O3c | rebuild 阈值收紧到 `1.25×live` | 22.63s | 0.88× | — | ✅ 相同 | ❌ 更差 |
| O3d | rebuild 阈值 `3×live`（扫描出的最优） | 19.57s | 1.02× | — | ✅ 相同 | ❌ 只 1.8%，低于 5% 阈值 |

**目标语料确认（owt_train 11.9GB + vocab 32000，`python cs336_basics/train_tinystories.py`）**：

| 阶段 | 优化前 | 优化后 | 加速 |
|---|---|---|---|
| Pre-tokenization | 40.84s | 41.53s | — |
| **BPE Merge** | **544.95s** | **236.54s** | **2.30×** |
| 端到端 | 585.8s | 278.1s | 2.11× |

⇒ 迭代语料预测 2.15×，目标语料实际 2.30×，**兑现率 107%**。两级语料的选法是对的（owt_valid 保留了 owt 的 unique pre-token 特征）。
merge 仍占端到端的 **85%**。

rebuild 阈值扫描（单次跑）：`1.25×` 22.63 / `1.5×` 20.25 / **`2×` 19.93（现值）** / `3×` 19.08 / `4×` 20.10 / `6×` 21.61 / `8×+1M` 27.79 / 不重建 51.82 ⇒ **曲线在 2~4× 之间很平，现值已在最优区间。**

#### 3.6.5b 并行上限要在优化之后重算（重要）

§3.6.3 里那个 **3.92×** 的并行上限是在**旧代码**上测的。O1 把 MAP 侧（每词的本地计算）砍掉了一大块，而 REDUCE 侧（`pair_counts` + heap）没动 ⇒ **可并行的比例变小了，上限跟着掉**。用当前代码重测（owt_valid，worker 侧 = new_seq + delta + word_pairs + 局部 p2w；parent 侧 = pc + heap）：

| 环节 | 耗时 | 占比 |
|---|---|---|
| worker 侧（可分片并行） | 13.9s | 67% |
| parent 侧（`pc` + heap push） | 5.9s | 28% |
| `pop_best` | 1.0s | 5% |

- Amdahl 上限（worker 完美 32×、通信免费）：**2.82×**（旧代码是 3.92×）
- 绝对上限（worker 归零）：2.99× ⇒ **parent 侧的 28% 是硬地板**，要突破得连 heap 也分片（每个 worker 维护局部 top-k，父进程只归并候选），复杂度和 tie-break 正确性风险都大一档。

**通信开销是固定的、和语料规模无关**：31743 轮 × 一次 32 进程 round-trip(~300µs) ≈ **9.5s**。
- owt_valid（整个 loop 才 20.8s）：9.5s 开销 > 省下的 13.5s 的大部分 ⇒ **净收益约 1.2×，不值得**。
- owt_train（loop 236.5s，同样 31743 轮）：每轮工作量大 11 倍，9.5s 只占 4% ⇒ 估算 236.5s → **~95s，约 2.5×**。
- ⇒ **同一份并行代码在两个语料上一个赚一个赔。轮数固定而每轮工作量随语料涨，是并行在这里能成立的唯一原因。**

#### 3.6.6 本轮的三条教训（比数字重要）

**1. GC 是隐藏的大头，而且会把正确的优化伪装成负收益。**
O1 单独上、GC 开着的时候是 **42.75 → 54.16s，看起来是 -27% 的负优化**，我差点直接回滚。关掉 GC 后才发现它其实是 28.21 → 19.22 = **+47%**。
- 原因：merge loop 每轮分配几百万个小容器（新序列、per-word pair dict、heap key 元组）。它们**不构成引用环**，靠引用计数就能回收；但分代 GC 仍会在每次 gen-2 扫描时遍历整个存活集合，而这个集合被 O1 的常驻 dict 撑大了。**分配得越多，GC 罚得越狠。**
- ⇒ **任何"多分配换少计算"的优化，评估前必须先把 GC 排除掉**，否则测到的是 GC 的账不是你的账。
- 实现上用 `try/finally` 包住，退出时恢复原状态，不要全局 `gc.disable()`。

**2. "空间换时间"不是无条件成立的，要看空间进了哪个数据结构。**
同一轮里两条都是花内存，结果相反：
- per-word pair 计数常驻（+2~3GB）→ **快 1.42×** ✅
- 让堆无限增长（+1.3GB）→ **慢 2.6×** ❌
- 区别在于：per-word dict 是 **O(1) 查找**，多占的内存不影响单次操作成本；而堆是 **O(log n)**，n 就是堆长度，放大空间直接推高每次 push/pop 的比较次数。**给对数级结构塞空间是自伤。**

**3. 每改一条就要重新 profile，别照着旧清单往下做。**
O2（heap key 缓存）是照着**改 O1 之前**的 profile 定的——那时 `_neg` 被调 4080 万次。O1 大幅减少了 touched pair 数量、连带减少了 push，heap 早就不是热点了，所以 O2 测出来 0 收益。**优化清单会被上一条优化作废。**

（另：微基准和整体跑可能给出**相反**的结论——O1b 的微基准是每词快 3.5×，整体却慢 27%。差异全在 GC 这种全局效应上。微基准只能用来解释局部，不能用来拍板。）
