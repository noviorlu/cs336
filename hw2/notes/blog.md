...
1. **先算纸面账**（§1）：训练静态显存 = 16 B × 参数量（🟦 权重 4 + 🟥 梯度 4 + Adam 状态 8），训练总 FLOPs = 6 × 参数量 × token 数。3.41B（xl）静态 50.8 GiB，5090 装不下；按 Chinchilla 的 20 token/参数 和实测 31% 算力利用率，0.97B（large）要训 41 天，单卡一周上限 ≈ 0.4B。
...
5. **显存大头是 attention 的 S、P，checkpoint 动不了**（§3）：🟩 里一层 3.6 GiB 有 56% 是 `[b, h, s, s]` 的分数矩阵，∝ seq²。它们是矩阵乘的输入，按 autograd 的规则「局部导数里有什么就存什么」必须存，融合省不掉。checkpoint 是前向不存、反向重算：🟩 从 32 层缩到只剩段输入，重算的那一层变成一段大小的 🟨，代价是整网多算一遍前向（step 慢 30%），S、P 在重算时照样要完整落显存。要消掉得改 attention 的 kernel——[第二篇](blog2.md) FlashAttention。
