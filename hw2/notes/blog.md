...
1. **先算纸面账再开机**（§1）
   - 现象：3.41B 参数的模型（作业的 xl）在 5090 的 31.3 GiB 上跑不了 full step；0.97B（large）能跑但训到收敛要 41 天。
   - 原因：训练静态显存 16 B/参数（fp32 权重 + 梯度 + AdamW 状态），xl 光这项 50.8 GiB；训练算力 ≈ 6 × 参数量 × token 数，按 Chinchilla 20 token/参数 和实测算力利用率 31% 反推。
   - 做法：单卡 5090 一周内能训完的上限约 0.4B（作业的 medium，8 天）。
2. **GPU 时间看的是「张量被搬了几遍」，不是 FLOPs**（§2.1）
   - 现象：seq 256 → 1024，前向里矩阵乘占比从 82% 掉到 54%，让出的全被 attention 里几乎不算数的操作吃掉：softmax FLOPs 只有 PV 矩阵乘的 1/5，时间却是它的 6×。
   - 原因：eager 模式每个算子是独立 kernel，都要把整个张量过一遍显存；这些 op 的算术强度远低于 ridge point，时间 = bytes / 带宽；分数矩阵 ∝ seq²，seq 翻 4 倍它们翻 16 倍、矩阵乘只翻 4 倍。
   - 做法：融合，让分数矩阵少落显存（fused softmax、FlashAttention）。测之前先空跑 1 步：首步含 ~300 ms 进程级一次性开销，不剔除均值虚高 7–59%。
3. **峰值显存落在哪一刻，看 activation 和梯度谁大**（§2.2）
   - 现象：small/medium/large 的训练峰值 = 权重 + 全部 activation，在前向末尾；xl@128 的峰值 = 2 × 权重，在反向末尾。纸面 16 B/参数 比实测多算一个权重的量。
   - 原因：`.grad` 不常驻——PyTorch 2.0 默认 `set_to_none=True`，step 后释放、反向逐层重建；反向每走一层释放一层 activation、新增一层梯度，activation 和梯度不会同时全在。
   - 做法：估峰值用 权重 + max(activation, 梯度)；activation ∝ token 数，正常训练都是 activation 大。
4. **bf16 混合精度：快 2×、省 20% 显存，代价是归约精度**（§2.3）
   - 现象：前向快 1.9–2.3×、反向 1.7–1.9×；训练峰值省 18–21%；但 `s += 0.01` 用 bf16 累加器到 4.0 就加不动。
   - 原因：加速 = bytes 减半 × 换算术单元（fp32 矩阵乘只能走 CUDA core，bf16 进 Tensor core）。显存只减矩阵乘相关的 activation，权重和梯度不变、还多一份 bf16 权重副本，所以推理式纯前向反而多 6.3 GiB。bf16 只有 7 位尾数，4.0 + 0.01 舍回 4.0。
   - 做法：归约类算子（LayerNorm / softmax / loss）留 fp32，autocast 默认如此；多卡用 Megatron 布局（bf16 参数常驻、fp32 master 放 optimizer 里分片）。
5. **显存大头是 attention 的分数矩阵，checkpoint 动不了它**（§3）
   - 现象：xl@2048 一层为反向存 3.6 GiB，56% 是 `[batch, heads, seq, seq]` 的 S = QKᵀ 和 P = softmax(S)，随 seq² 长；32 层 114 GiB。
   - 原因：autograd 的规则是「局部导数里出现什么就存什么」，S、P 是矩阵乘的输入，融合也省不掉。checkpoint 是推迟不是压缩：前向只留段输入、反向重算段内张量，恰好多一次前向（实测 +28–33%），峰值仍含一整段的 S、P；每层一段最省，没有 U 形。
   - 做法：checkpoint 的旋钮是「包几层」，按显存缺口取最小；要消掉 S、P 本身得改 attention 的 kernel——[第二篇](blog2.md) FlashAttention。
