...
|:-----|:--|--:|:--|--:|:--|--:|--:|
| small  |  17.2 →  9.2 | 1.87× |  33.9 →  20.1 | 1.69× | 4.08 → 3.18 (−21%) | +0.24 | −1.14 |
...
峰值显存是两项相抵：权重侧**变多**——fp32 原件不动，autocast 再存一份 bf16 副本（+参数量 × 2 B）；activation 侧**减半**——autograd 为反向存的是 bf16 版本（表中 activation 侧 = 峰值差 − 副本）。训练时 activation 远大于权重所以净减，模型越大副本越占上风；到 xl@128 这种 token 少、权重大的配置就持平（fwd_bwd）甚至 +49%（`no_grad` 推理式前向，没有 activation 可省）——见 §2.4(c)。
