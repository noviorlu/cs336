...
| P = softmax(S)（hw1 版：max、减、exp、sum、除 5 个 kernel） | 5 × 6.7e7 = 3.4e8（exp 按 1 次算；它走 SFU，吞吐是 FMA 的 1/8，即便记 20 次也只到 1.3e9、I ≈ 0.6，结论不变） | 5 个 kernel 各读 1–2 遍、写 1 遍 S 大小的张量，合计 8 × 256 MiB = 2 GiB（末次写出的即 P） | 0.16 | ~0 | **1.2 ms** | 1.35 ms |
