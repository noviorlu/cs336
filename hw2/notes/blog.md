...
**先说显存的判据**。训练一步的峰值落在哪一刻，决定了 bf16 能不能省出来。记 W = 权重、G = 全部梯度、A = 前向结束时为反向存的全部 saved tensors。W 和 G 在 autocast 下都还是 fp32（(b) 表里梯度 fp32；autocast 只转算子的输入，不动参数和 `.grad`），所以 G 的大小恒等于 W。反向从最后一层往前走，到第 k 层（共 L 层）时活着的是：
