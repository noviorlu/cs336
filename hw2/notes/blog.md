...
训练静态 = 16 B/参数（fp32 权重 4 + 梯度 4 + Adam m/v 8），autocast 再 +2；activation（为反向存的 saved tensors，§2.3 记作 A）由 §2.2 实测反推：带图前向峰值 − 权重。单位 GiB：
...
|:--|--:|--:|--:|--:|--:|--:|
| 普通 Linear 作参照：y = x·Wᵀ（FFN 的 w1，x `[4096 token, 1024]`，W `[4096, 1024]`，y `[4096, 4096]`） | 2·4096·1024·4096 = 3.4e10 | 读 x 16 MiB、W 16 MiB，写 y 64 MiB | 340 | **0.32 ms** | 0.06 ms | ≈ 0.5 ms |
| S = QKᵀ（Q、K 各 `[64 个 batch×head, 1024, 64]`，S `[64, 1024, 1024]`） | 64·(2·1024·64·1024) = 8.6e9 | 读 Q、K 32 MiB，写 S 256 MiB | 28 | 0.08 ms | **0.17 ms** | 0.61 ms |
| S / √d（S 原地缩放） | 1 × 6.7e7 个元素 = 6.7e7 | 读 S 256 MiB，写 S 256 MiB | 0.13 | ~0 | **0.30 ms** | 0.34 ms |
| masked_fill（causal mask 填 −inf） | 0 | 读 S 256 MiB，写 S 256 MiB | 0 | 0 | **0.30 ms** | 0.35 ms |
| P = softmax(S)（hw1 版：max、减、exp、sum、除 5 个 kernel） | 5 × 6.7e7 = 3.4e8 | 每个 kernel 读写一遍 S 大小的张量，合计 8 遍 2 GiB，最后写出 P | 0.16 | ~0 | **1.2 ms** | 1.35 ms |
| O = PV（P `[64, 1024, 1024]`，V `[64, 1024, 64]`，O `[64, 1024, 64]`） | 64·(2·1024·1024·64) = 8.6e9 | 读 P 256 MiB、V 8 MiB，写 O 8 MiB | 30 | 0.08 ms | **0.16 ms** | 0.24 ms |

FLOPs 怎么算：矩阵乘 `[M,K]×[K,N]` 是 2·M·N·K（每个输出元素 K 次乘加，乘、加各算 1 FLOP），batched 再乘个数；逐元素 op 是「每元素几次运算 × 元素数」，S 有 4·16·1024² = 6.7e7 个元素，softmax 每元素约 5 次（max、减、exp、sum、除），mask 是选择不算运算记 0。
...
四列各是什么，用 §2.3 的记号（A = 为反向存的 saved tensors，G = 梯度 = W）：
