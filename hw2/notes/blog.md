...
|:--|--:|--:|
| 存的参数本身（已在权重里，不额外占） | 0.45 GiB（fp32 W） | 0 |
| bf16 权重副本 | 0 | **+0.23** |
| activation | 3.41 | **2.28**（bf16 1.34 + 仍为 fp32 的 0.93） |
| 峰值 | 4.07 | 3.18 |

峰值变化 −0.89 = 副本 +0.23 + activation −1.13（差 0.02 是反向临时量）。权重侧**变多**：autocast 把 fp32 W 转成 bf16 再做矩阵乘，反向要用这份 bf16 版，autograd 存的就是它——fp32 W 反而不再被存，但它本来就在显存里，所以副本是净增。activation 侧**减少**：矩阵乘的输入输出都成了 bf16，但不是严格减半（3.41 → 2.28 = 67%），因为 RMSNorm、残差流、loss 这些 autocast 不碰的张量还有 0.93 GiB 留在 fp32。训练时 activation 远大于权重所以净减，模型越大副本越占上风；到 xl@128 这种 token 少、权重大的配置就持平（fwd_bwd）甚至 +49%（`no_grad` 推理式前向，没有 activation 可省）——见 §2.4(c)。
