...
**图 2.2-2** xl 的显存快照，每个点是一次真实分配 / 释放：左上 seq 128 纯前向、右上 seq 2048 纯前向、下 seq 128 full step（红线为阶段分界，按分配时的 Python 栈判断）
...
- (b) full 任何 seq 都装不下（optimizer 的 2W）；seq 2048 连 fwd_bwd 都过不了前向，第 2 层的尖峰就撞 25.96——第二篇 FlashAttention 消的就是它。
