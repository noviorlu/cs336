...
|:--|:--|:--|:--|
| W | 权重 | fp32（autocast 只转算子输入，不动参数） | 常数 |
| G·k/L | 已累加进 `.grad` 的梯度；反向中间量是 bf16，最后 cast 成 fp32 累加，所以 G 的大小 = W | fp32 | 每层 +G/L |
| A·(L−k)/L | 前向存的、还没用掉的 saved tensors（含 bf16 权重副本） | 矩阵乘相关 bf16，RMSNorm/残差流 fp32 | 每层 −A/L |
| T | 这一层反向的临时量：`dy`、`dx`、累加前的 `dW`、softmax/SiLU 反向的中间张量，算完即释放 | bf16 | 常数（一层的量，§2.2(e) 里 xl ≈ 0.95 GiB 分配即释放，对峰值贡献 ~0.2） |
