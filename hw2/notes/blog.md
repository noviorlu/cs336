...
**先说显存的判据**。训练一步的峰值落在哪一刻，决定了 bf16 能不能省出来。模型 L 层，反向从最后一层往前走，已经走完 k 层（k 从 0 到 L）时，显存 M(k) 由四项组成：

<p align="center">$M(k) = W + G \cdot k/L + A \cdot (L-k)/L + T$</p>

| 项 | 是什么 | dtype（autocast 下） | 随 k 怎么变 |
|:--|:--|:--|:--|
| W | 全部权重的大小 | fp32（autocast 只转算子输入，不动参数） | 常数 |
| G·k/L | G = 全部参数梯度的大小（= W，因为 `.grad` 和参数同 dtype）；走完 k 层就累加了 k/L 份。反向中间量是 bf16，最后 cast 成 fp32 累加进 `.grad` | fp32 | 每层 +G/L |
| A·(L−k)/L | A = 前向结束时为反向存的全部 saved tensors（含 bf16 权重副本）；每走完一层释放一层，剩 (L−k)/L | 矩阵乘相关 bf16，RMSNorm/残差流 fp32 | 每层 −A/L |
| T | 正在算的这一层反向的临时量：`dy`、`dx`、累加前的 `dW`、softmax/SiLU 反向的中间张量，算完即释放 | bf16 | 常数（一层的量，§2.2(e) 里 xl ≈ 0.95 GiB 分配即释放，对峰值贡献 ~0.2） |

T 是常数偏移，曲线涨还是降只看 **A 和 G 谁大**：
