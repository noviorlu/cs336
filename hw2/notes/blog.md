...
| P = softmax(S)（hw1 版 5 个 kernel：max、减 max、exp、sum、除） | 每元素 1+1+1+1+1 = 5 次 × 6.7e7 = 3.4e8（exp 走 SFU、除法 = 倒数 + 修正，按硬件代价记 20 和 4 次也只到 1.8e9，I ≈ 0.85） | 5 个 kernel 各读 1–2 遍、写 1 遍 S 大小的张量，合计 8 × 256 MiB = 2 GiB（末次写出的即 P） | 0.16 | ~0 | **1.2 ms** | 1.35 ms |
