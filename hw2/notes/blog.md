...
**图 2.2-2** xl 的显存快照，每个点是一次真实分配 / 释放：左上 seq 128 纯前向、右上 seq 2048 纯前向、左下 seq 128 full step（红线为阶段分界，按分配时的 Python 栈判断）、右下 seq 2048 fwd_bwd 记录到 OOM 为止

- (a) 三个阶段靠**斜率**认：前向 32 级上坡（每层留 166 MiB，12.8 → 18.1）；反向继续爬到 25.5（每层 −166 +410）；optimizer 垂直冲到 ~28.6 OOM。前两段和图 2.2-1 左的模型一致。纯前向不留东西，seq 128 平，seq 2048 的 32 根尖峰是每层 attention 的 2 GiB 分数矩阵链，见 (d)。
- (b) full 任何 seq 都装不下（optimizer 的 2W）。seq 2048 连 fwd_bwd 都过不了前向（图 2.2-2 右下）：第 1 层的 attention 链冲到 21.6 后回落到 17.5——这一层留下了 ~4.7 GiB saved tensors——第 2 层的链再往上爬，25.96 撞墙。第二篇 FlashAttention 消的就是这根尖峰。
