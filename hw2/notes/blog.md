...
1. **先算纸面账再开机**（§1）
   - 现象：3.41B 参数的模型（作业的 xl）在 5090 的 31.3 GiB 上任何配置都跑不了 full step；0.97B（large）能跑但训到收敛要 41 天。
   - 原因：训练静态显存 16 B/参数（fp32 权重 4 + 梯度 4 + AdamW 状态 8），xl 光这项 50.8 GiB；训练算力 ≈ 6 × 参数量 × token 数，按 Chinchilla 20 token/参数 和实测算力利用率 31% 反推。
   - 做法：单卡 5090 一周内能训完的上限约 0.4B 参数（作业的 medium，8 天）；更大的要多卡或换精度。
2. **不预热的计时数字没意义**（§2.1）
   - 现象：进程第一步比稳态慢 1.7–6.9×，绝对值 ~300 ms 与模型大小无关；平均进去后均值虚高 7–59%、标准差涨 100 倍。
   - 原因：第一步做的是进程级一次性开销——GPU kernel 首次加载、cuBLAS 建句柄、显存池首次向驱动要内存——和模型算得快慢无关；不剔除，比的是「谁先启动」。
   - 做法：先空跑 1 步再计时。`timeit` + `synchronize()` 回答「多快」，Nsight Systems 回答「时间花在哪」。
3. **GPU 时间看的是「张量被搬了几遍」，不是 FLOPs**（§2.1）
   - 现象：seq 256 → 1024，前向里矩阵乘占比从 82% 掉到 54%，让出的全被 attention 里几个几乎不算数的操作吃掉：softmax（hw1 版 5 个 kernel，把 256 MiB 的分数矩阵读写 8 遍）FLOPs 只有 PV 矩阵乘的 1/5，时间却是它的 6×。
   - 原因：eager 模式每个算子是独立 kernel，都要把整个张量过一遍显存；这些 op 的算术强度远低于 5090 的 ridge point 60，时间 = bytes / 带宽；分数矩阵 ∝ seq²，seq 翻 4 倍它们翻 16 倍、矩阵乘只翻 4 倍。
   - 做法：融合，让分数矩阵少落显存——fused softmax 8 遍 → 2 遍，FlashAttention 不落。
4. **峰值显存落在哪一刻，看 activation 和梯度谁大**（§2.2）
   - 现象：small/medium/large 的训练峰值 = 权重 + 全部 activation，出现在前向末尾；xl@128 的峰值 = 2 × 权重，出现在反向末尾。纸面账 16 B/参数 比实测多算一个权重的量。
   - 原因：`.grad` 不是常驻缓冲区——PyTorch 2.0 默认 `set_to_none=True`，step 后释放、反向时逐层重建；反向每走一层释放一层 activation、新增一层梯度，谁大曲线就往谁那边走，activation 和梯度不会同时全在。
   - 做法：估峰值用 权重 + max(activation, 梯度)；activation ∝ token 数，正常训练都是 activation 大。
5. **显存大头是 attention 的分数矩阵**（§2.2 / §3.2）
   - 现象：xl@2048 前向最大的单笔分配是 2 GiB，一层里连造 6 份；融合后一层为反向存的 3.6 GiB 里 56% 仍是 S = QKᵀ 和 P = softmax(S)。
   - 原因：两者形状 `[batch, heads, seq, seq]`，随 seq² 长；其它张量都 ∝ seq。
   - 做法：checkpoint 动不了它们（重算时仍要物化），只能改 attention 的 kernel——第二篇 FlashAttention。
6. **bf16 混合精度：快 2×、省 20% 显存，代价是归约精度**（§2.3）
   - 现象：前向快 1.9–2.3×、反向 1.7–1.9×，模型越大越快；训练峰值省 18–21%；但 `s += 0.01` 用 bf16 累加器到 4.0 就加不动。
   - 原因：加速 = 搬的 bytes 减半 × 换算术单元（fp32 矩阵乘只能走 CUDA core，bf16 进 Tensor core，5090 快 2×）。显存只减矩阵乘相关的 activation（≈ 减 1/3），权重和梯度不变、还多一份 bf16 权重副本；副本只在前向期间存在，所以推理式的纯前向反而多 6.3 GiB。精度：bf16 只有 7 位尾数，4.0 + 0.01 舍回 4.0。
   - 做法：归约类算子（LayerNorm / softmax / loss）留 fp32，autocast 默认如此，代价 2–7%；多卡时用 Megatron 布局（bf16 参数常驻、fp32 master 放 optimizer 里分片）。
7. **autograd 为反向存什么**（§3.1）
   - 现象：RMSNorm 拆成 5 个算子，pack 了 6 次却只多占一份输入大小的显存。
   - 原因：一个算子的局部导数里出现什么张量就存什么，存的是引用不是拷贝；本来就在的输入和参数不额外占。
   - 做法：`torch.compile` 把 5 个算子融成 1 个后连 x̂ 也不存、反向重算——「用重算换显存」在单算子层面就成立。
8. **激活检查点是推迟不是压缩**（§3.2）
   - 现象：不管怎么分段，step 都恰好多算一次前向（实测慢 28–33%）；显存随段长单调，每层一段最省，没有 U 形。
   - 原因：前向只留每段的输入，反向到该段时重算段内张量；峰值 = 所有段输入 + 一段的中间张量，Transformer 每层的中间张量远大于层输入，段输入再多也压不过一段。
   - 做法：段长永远 1，旋钮是「包几层」，按显存缺口取最小；顺序：FlashAttention / 选择性重算 → 包 N 层 → 全包。递归二分能到 O(log L) 显存但计算 O(L log L)，实践不用。
